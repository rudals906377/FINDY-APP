...
- 앱 기능이면 `FINDY.py`에 실제 연결되어 있는지 먼저 봅니다.
...
3. 실제 실행 경로인 `FINDY.py`의 렌더링 분기에 연결합니다.
4. `smoke_test.py`의 `files` 리스트에 새 파일을 추가합니다.

## 현재 정리 방향

- `FINDY.py`는 당장 쪼개기보다 현재 동작을 기준으로 안정화합니다.
