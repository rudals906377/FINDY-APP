...
- `python_files/FINDY.py`: 예약형 메인 앱
- `python_files/FINDY2.py`: 첫 배포용 커뮤니티형 앱
...
├── python_files/               # 실제 파이썬 앱 본문과 보조 스크립트
├── web/                        # FINDY 웹사이트
└── docs/                       # 앱별/프로젝트별 문서
```
...
- 앱 기능이면 `python_files/FINDY.py`에 실제 연결되어 있는지 먼저 봅니다.
...
3. 실제 실행 경로인 `python_files/FINDY.py`의 렌더링 분기에 연결합니다.
4. `python_files/smoke_test.py`의 `files` 리스트에 새 파일을 추가합니다.
5. 관련 기준 문서를 `docs/apps/` 또는 `docs/project/`에서 함께 업데이트합니다.

## 현재 정리 방향

- `python_files/FINDY.py`는 당장 쪼개기보다 현재 동작을 기준으로 안정화합니다.
...
- FINDY2는 첫 배포용 커뮤니티 앱 기준이며, 안정화된 디자인/흐름은 이후 FINDY에 반영합니다.
