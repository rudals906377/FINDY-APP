...
def _status_badge(status: str):
    ...
    bg = "#F3ECE3" if status == "접수 완료" else "#EDF7EF"
    return ft.Container(
        padding=ft.padding.symmetric(horizontal=10, vertical=5),
        border_radius=999,
        bgcolor=bg,
        content=ft.Text(status, size=10, color=color, weight=ft.FontWeight.W_600),
    )
            ...
            controls=[
                ft.Row(
                    controls=[
                        ft.Column(
                            controls=[
                                ft.Text(item.get("title", "제목 없음"), size=15, weight=ft.FontWeight.BOLD, color="#1F1F1F"),
